...
\par Template parameters
	...
	- T type of input data (can be any: float, double, int, unsigned int, etc...)
...
public:
	...
	*/
	THistogram(unsigned int nCounters = 10);
	...
	*/
	...
	void Resize( unsigned int nCounters );
...
protected:
	//! Computes the step
	void ComputeStep()								{	m_dStep = (TOut)(((TOut)(m_tMax-m_tMin)) / (m_vCounters.size()));};
	//! Data accumulators
	std::vector<unsigned int> m_vCounters;
	//! minimum of dataset
	T m_tMin;
	//! maximum of dataset
	T m_tMax;
...
THistogram<T,TOut>::THistogram(unsigned int nCounters)
...
template<class T, class TOut>
void THistogram<T,TOut>::Resize( unsigned int nCounters )
{
	Clear();
...
{	
	...
	if (index >= int(m_vCounters.size()) || index < 0)
		return;
...
std::vector<TOut> THistogram<T,TOut>::GetHistogramD() const
{
	...
	for (unsigned int i = 0;i<m_vCounters.size(); i++)
