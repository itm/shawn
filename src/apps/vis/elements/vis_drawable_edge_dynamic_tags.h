 ...
 ************************************************************************/
#ifndef __SHAWN_TUBSAPPS_VIS_DRAWABLE_EDGE_DYNAMIC_TAGS_H
#define __SHAWN_TUBSAPPS_VIS_DRAWABLE_EDGE_DYNAMIC_TAGS_H
...
{
	/**
	 * This class is intended to draw edges based on tags
 	 * attached to processors. So each node can contain
	 * one tag for each node it is connected to.
	 *
	 * @sa EdgePropertySet
	 * @sa CreateEdgesTask
	 */
   class DrawableEdgeDynamicTags
      : public DrawableEdge
   {
   public:
      ///@name Contructor/Destructor
      ///@{
      DrawableEdgeDynamicTags( const shawn::Node&,
                           const shawn::Node&,
                           const std::string& p = PREFIX,
                           const std::string& np = DrawableNodeDefault::PREFIX,
                           const std::string tagname_regex = ".*");
      virtual ~DrawableEdgeDynamicTags();
      ///@}
      ...
      virtual void init( void ) throw();
	   ...
	   /**
       * Draws the edge(s) onto the given cairo surface.
       */
   ...
   private:
      ...
      const std::string tagname_regex_;
   };
}
